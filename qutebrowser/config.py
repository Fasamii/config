...
c.editor.command = ['kitty', 'nvim', '{file}', '+normal {line}G{column0}l'] #, '+startinsert']

c.fileselect.handler = "default"
# # c.fileselect.single_file.command = ["kitty", "sh", "-c", 'fzf --no-multi > "$1"', "--", "{}"]
# c.fileselect.single_file.command = [
#     "kitty", "sh", "-c",
#     'fd . --type f | fzf --preview "bat --style=numbers --color=always --line-range :500 {}" > "$1"',
#     "--", "{}"
# ]
# c.fileselect.multiple_files.command = ["kitty", "sh", "-c",'fzf --multi > "$1"', "--", "{}"]
c.fileselect.single_file.command = ["kitty", "--class", "fzf-float", "-e", "sh", "-c", "cd ~ && find . -type f | fzf --preview '~/.config/util/scripts/preview.sh {}' --preview-window=up:50% > /tmp/qute_filepicker && cat /tmp/qute_filepicker"]
c.fileselect.multiple_files.command = ["kitty", "--class", "fzf-float", "-e", "sh", "-c", "cd ~ && find . -type f | fzf --multi --preview '~/.config/util/scripts/preview.sh {}' --preview-window=up:50% > /tmp/qute_filepicker && cat /tmp/qute_filepicker"]
...
c.zoom.levels = [10, 100, 300]  # More granular zoom levels
c.auto_save.session = True # save session automatically 
c.session.lazy_restore = True # restore session automatically
c.auto_save.interval = 15000 ## Time interval (in milliseconds) between auto-saves of config/cookies/etc.
c.downloads.location.prompt = True
c.downloads.location.directory = "~/Downloads/" # path to default downloads directory
c.downloads.open_dispatcher = os.path.expanduser("~/.config/util/scripts/file-opener.sh")
config.set('zoom.mouse_divider', 1000) # TODO: adjust that setting and make others to make zooming more smooth
c.completion.web_history.max_items = 100000
c.content.cookies.accept = "no-3rdparty"
# Tabs
config.set("tabs.show", "multiple") # show tabs when there is multiple (hide if only one)
config.set("tabs.background", False) # focus on new tabs
c.tabs.select_on_remove = "prev"
config.set("tabs.new_position.related", "next") # open tabs right under related tabs
config.set("tabs.new_position.unrelated", "last") # open tabs right under unrelated tabs
config.set("tabs.new_position.stacking", False) # logically stack tabs (if parent tab closes stacked tabs also)
