...
config.set('content.headers.user_agent', (  # TODO: check if that even work
...
c.fileselect.handler = "default"  # TODO: change to your custom commands
c.fileselect.single_file.command = ["kitty", "--class", "fzf-float", "-e", "sh", "-c",
                                    "cd ~ && find . -type f | fzf --preview '~/.config/util/scripts/preview.sh {}' --preview-window=up:50% > /tmp/qute_filepicker && cat /tmp/qute_filepicker"]
c.fileselect.multiple_files.command = ["kitty", "--class", "fzf-float", "-e", "sh", "-c",
                                       "cd ~ && find . -type f | fzf --multi --preview '~/.config/util/scripts/preview.sh {}' --preview-window=up:50% > /tmp/qute_filepicker && cat /tmp/qute_filepicker"]
...
c.downloads.open_dispatcher = os.path.expanduser(
    "~/.config/util/scripts/file-opener.sh")
# TODO: adjust that setting and make others to make zooming more smooth
config.set('zoom.mouse_divider', 1000)
